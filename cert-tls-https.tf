...
resource "kubernetes_secret" "tls_route53_secret" {
  metadata {
    name      = "project-prod-route53-credentials-secret"
...
resource "null_resource" "crd_cert_resources_install" {
  ...
  provisioner "local-exec" {
    command = "echo INFO: installing CRD... && . ./my-kubectl.sh apply -f https://raw.githubusercontent.com/jetstack/cert-manager/${local.jetstack_cert_crd_version}/deploy/manifests/00-crds.yaml && echo INFO: complete, sleeping 10 sec... && sleep 10"
  }
