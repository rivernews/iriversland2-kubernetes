...
EOT
  ...
  provisioner "local-exec" {
    ...
    command = self.triggers.letsencrypt_env == "prod" ? "echo && echo && echo INFO: will not delete secret for production letsencrypt due to quota concern. Please manually delete secret using kubectl if necessary." : join("\n", [
        ...
        "bash ./my-kubectl.sh delete secrets ${self.triggers.central_tls_ing_certificate_secret_name} -n ${self.triggers.cert_manager_namespace}",
        ...
        "bash ./my-kubectl.sh delete secrets ${self.triggers.cert_cluster_issuer_secret_name_prod} ${self.triggers.cert_cluster_issuer_secret_name_staging} -n ${self.triggers.cert_manager_namespace}",
