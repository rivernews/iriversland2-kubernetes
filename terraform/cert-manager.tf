...
resource "kubernetes_secret_v1" "tls_route53_secret" {
  metadata {
...
resource "null_resource" "crd_cert_resources_install" {
  triggers = {
    ...
    cert_manager_namespace = kubernetes_namespace_v1.cert_manager.metadata.0.name
    ...
    tls_route53_secret_name      = kubernetes_secret_v1.tls_route53_secret.metadata.0.name
...
spec:
  acme:
    ...
    #             name: ${kubernetes_secret_v1.tls_route53_secret.metadata.0.name}
...
EOT
  ...
  provisioner "local-exec" {
    ...
    command = self.triggers.letsencrypt_env == "prod" ? "echo && echo && echo INFO: will not delete secret for production letsencrypt due to quota concern. Please manually delete secret using kubectl if necessary." : join("\n", [
        ...
        # "kubectl delete secrets ${local.cert_cluster_issuer_secret_name_prod} ${local.cert_cluster_issuer_secret_name_staging} -n ${kubernetes_namespace_v1.cert_manager.metadata.0.name}",
